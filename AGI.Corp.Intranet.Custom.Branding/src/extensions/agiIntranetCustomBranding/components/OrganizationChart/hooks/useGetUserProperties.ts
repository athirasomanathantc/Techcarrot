...
import { IPersonProperties, IUserInfo } from "../../../models";
...
type getUserProfileFunc = (currentUser: string,
  startUser?: string,
  showManagers?: string) => Promise<returnProfileData>;
...
const getExtendedManagers = async (
  ...
  for (const manager of extendedManagers) {
    if (showManagers === 'none' && manager !== startUser) {
      continue;
    }
  ...
  await batch.execute();
  return (showManagers === 'one') ? wManagers.slice(-1) : wManagers;
};
