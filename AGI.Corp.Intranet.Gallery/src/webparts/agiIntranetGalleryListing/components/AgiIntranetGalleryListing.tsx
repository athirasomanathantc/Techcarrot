...
export default class AgiIntranetGalleryListing extends React.Component<IAgiIntranetGalleryListingProps, IAgiIntranetGalleryListingState> {
  ...
  private async getImageGalleryItems(subFolderName): Promise<void> {
    debugger;
    // sp.web.lists.getByTitle('Image Gallery').items.select('*, FileRef, FileLeafRef').get().then((items: IImageItem[]) => {
    //   //const changeAgentItems = items && items.length > 0 ? items[0] : NULL_ABOUT_CHANGEAGENT_ITEM;
    //   console.log("Content Itms",items);
    //   this.setState({
    //     videoItems: items
    //   });
    // });
